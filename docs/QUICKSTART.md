...
Теперь проверьте связку целиком. Создайте тестовый документ, напишите в нём три-четыре абзаца и оставьте два комментария с просьбами поправить текст — так, как их оставил бы заказчик. Дайте агенту ссылку и скажите: «отработай комментарии».

Вот весь цикл, который он пройдёт. Команды видны в терминале, и по ним понятно, что происходит:

```bash
skrepka comments DOC --output comments.json     # 1. прочитать треды
skrepka patch DOC ops.json --dry-run            # 2. примерить правки
skrepka patch DOC ops.json                      # 3. применить
skrepka reply DOC --file replies.json           # 4. ответить в треды
```

`DOC` — идентификатор или ссылка на документ.

**Шаг 2 ничего не записывает.** Он говорит по каждой правке, применится ли она: `would_apply` — да; `would_refuse` — нет, и видно почему; `unknown` — по чтению не решается, решит сама запись. Последнее на документе с комментариями обычное дело и не повод не пробовать.

**Шаг 3 — единственный, который меняет документ.** После него откройте документ и посмотрите глазами: текст поправлен, комментарии на месте, ни один тред не закрыт. Закрывать их — ваше дело, агент этого не делает.

Ответ приходит после правки, а не до неё: пока правка не сделана, в треде нечего сказать.
