...
Гайд рассчитан на то, что skrepka уже установлена. Если ещё нет, поставьте её по [README](../README.md).
...
Аккаунт Google (личный Gmail подойдёт), браузер, терминал и 15–30 минут.

Аккаунт может быть любым, не обязательно личным. Можно завести отдельный Gmail под агента — тогда проверьте, что у этого профиля есть доступ ко всем нужным документам.

Google периодически меняет консоль, поэтому кнопки могут стоять чуть иначе, чем на снимках. Порядок шагов от этого не меняется.
...
Включить API — значит разрешить проекту обращаться к службе. Доступ к вашим файлам это ещё не выдаёт: его вы дадите на шаге 6, на экране согласия.
...
Появится окно «Verification required», нажмите Continue. Поля про justification, demo-видео и предложение отправить приложение на проверку заполнять не нужно: для личного использования проверка Google не требуется (см. 4.4).

![Verification required — Continue](img/quickstart/08-verification-popup.png)

Затем прокрутите страницу вниз и нажмите Save, иначе доступ не сохранится. Появится «Data access changes saved!».

### 4.3. Опубликовать приложение (Publish app)

Слева откройте Audience. Если в Publishing status стоит Testing, нажмите Publish app и подтвердите Confirm в окне «Push to production».

![Publish app](img/quickstart/09-publish-app.png)

![Push to production — Confirm](img/quickstart/10-push-to-production.png)

В режиме Testing вход протухает каждые 7 дней, в Production живёт долго. Публикация не делает приложение «проверенным», но для личного использования этого и не требуется.

### 4.4. Плашку «требуется верификация» можно игнорировать

Из-за доступа `drive` консоль будет предлагать отправить приложение на проверку Google. Для личного использования это не нужно: вы пользуетесь своим приложением сами. Единственное следствие — на шаге 6 при входе Google покажет экран «приложение не проверено».
...
Доступ у агента теперь есть, но работать с документами он ещё не умеет. Подключите навыки skrepka по инструкции [PLUGIN.md](PLUGIN.md): они научат Claude Code или Codex читать комментарии, отвечать на них и вносить правки.

Потом проверьте связку на живом примере: создайте тестовый документ, оставьте в нём пару комментариев и попросите агента их отработать.
