...
skrepka убирает это копирование. Вы оставляете по тексту обычные комментарии: про структуру, логику, формулировки. Потом зовёте агента в документ, он читает треды, отвечает и правит текст на месте. Заказчик приходит, комментирует там же, работа продолжается тем же способом. Комментарии при этом не рвутся — даже если прокомментированный абзац переписан целиком, тред остаётся и переезжает на новый текст. Закрывает треды только человек.
