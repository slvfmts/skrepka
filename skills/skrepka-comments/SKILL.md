...
накрывшую якорь, она сама сужает до фрагмента, который реально меняется, а замену,
которая только дописывает текст, выполняет как вставку. Если в ответе есть
`op_notes` с `applied_as`, применённая операция текстуально не та, что ты просил, —
результат в документе тот же, но скажи об этом человеку.

Что остаётся отказом: меняется **весь** прокомментированный текст без остатка
(уцелеть нечему, тред стал бы призраком); в документе есть комментарии-призраки;
правка лезет внутрь таблицы с комментарием; в документе есть непринятые
предложения, а операция — замена или удаление. В отказе есть ссылка `?disco=` на
мешающий тред — дай её человеку, по ней тред открывается прямо в документе.
