...
1. **Прочитай комментарии.** Для больших тредов пиши в файл, а не в stdout —
   ...
   **В документе два разных разговора, и ты участвуешь не в обоих.** Комментарии
   ...
   не додумывай по названию или первому кандидату: несколько/ноль совпадений,
   отсутствующая цитата или сломанный набор tabId не доказывают вкладку.
   `candidates` — подсказка для проверки человеком, не разрешение отвечать.
   `status: document` — отдельный комментарий уровня всего документа, а не
   неизвестная вкладка. При просьбе обработать одну вкладку бери только
   `exact` с нужным `tab_id`; остальные покажи человеку как неопределённые.

   Отдельно смотри на `anchor_export`. `status: ghost` — консервативный
   read-only вердикт: запись отсутствует в стабильном чтении, экспорт содержит
   запись новее последней активности этого треда (включая resolve/reopen), а
   цитаты нет ни в одной вкладке. Назови тред человеку со ссылкой, но не
   удаляй. `status: unknown` не превращай в призрака по догадке: так честно
   обозначаются в том числе гонка снимков и нечитаемый author/date экспорта.
   И особенно не читай `record_present` как «якорь жив сейчас»: это только
   запись в read-only DOCX-выгрузке, чья свежесть не закреплена canary, поэтому
   рядом стоит `export_freshness: unproven`. Сам `comments` делает только
   чтения — никаких canary, ответов или иных записей.
...
2. **Ответь** на треды по делу. Ответ и правку можно делать одним проходом — не жди,
   ...
   Если ты только что по ошибке отправил лишний обычный ответ, удаляется именно
   он, а не весь разговор: перечитай полный `comments`, возьми точные `comment_id`
   и `reply_id` и вызови `skrepka unreply <doc_id> <comment_id> <reply_id>`.
   `author.me: true` означает лишь текущий OAuth-аккаунт, а не доказывает, что
   текст написал агент: не удаляй старые «свои» ответы по догадке. Используй
   `unreply` только для ответа, который сам отправил в текущем проходе, либо по
   явной просьбе человека удалить названный reply.
...
3. **Поправь текст** через якорно-безопасный `patch`. Операции описываются в
   ...
   Перед записью можно запросить ограниченный advisory-прогон:

   ```
   skrepka patch <doc_id> ops.json --dry-run [--output receipt.json]
   ```

   Это read-only путь: он не вызывает `batchUpdate`, replies/deletes, canary
   или export. `would_apply`/`noop` означают только bounded решаемость на
   snapshot; для destructive правки комментария receipt честно сообщает
   `unknown` с причиной `fresh_anchor_map_requires_canary`, а зависимые
   последующие операции — `not_simulated` с `depends_on`. Exit 0 бывает только
   если все операции `would_apply` или `noop`; иначе exit 3.

   Поддержаны `replace_anchor` (точный `comment_id` + `with`),
   `replace_quote` / `replace_range`, `insert_before_quote|range`,
   ...
   Для `replace_anchor` передавай id корневого активного треда:

   ```json
   {"op": "replace_anchor", "comment_id": "AAACFwKC8_w", "with": "о"}
   ```

   Диапазон берётся только из свежего DOCX marker после canary; stale
   `quotedFileContent`, occurrence и догадки по одинаковому тексту не
   используются. Нужен ровно один доступный неразрешённый тред и доказанная
   вкладка (`--tab` для root/child tabs). Неоднозначный или повреждённый
   marker, гонка census, suggestion, named-range fence либо пустая замена
   (она осиротит сам тред) дают отказ до смысловой записи.

   Если нужно заменить более широкий фрагмент, который строго содержит этот
   anchor, используй `replace_around_anchor` и обязательно назови новый
   фрагмент треда с UTF-16 offset:
   ...
   хотя бы один положителен. `quote` — exact witness вычисленного диапазона,
   не глобальный поиск, поэтому одинаковые внешние строки допустимы.
   `anchor.text` обязан совпасть с `with` ровно по `anchor.start_utf16`;
   повторяющийся текст без offset не выбирается.
   Offset считается в UTF-16, внутрь emoji попадать нельзя. Skrepka получает
   старый anchor только из свежего marker #52, сначала переписывает сам
   anchor, затем отдельно меняет левую и правую части в одном
   revision-pinned batch. Пустой новый anchor, чужой anchor, protected/named
   range, suggestion или гонка дают отказ до смысловой записи. Старый anchor
   из одного code point (включая emoji) явно отдаётся в UI: безопасной
   внутренней rewrite boundary для него нет.

   Если `quote` и `with` уже совпадают, no-op допустим только при exact
   совпадении `anchor.start_utf16` с относительным fresh-marker offset и
   `anchor.text` с marker. Выбор другой одинаковой копии — bounded refusal,
   не тихий no-op. Suggestions и freshness остаются gates; после них true
   no-op не блокируется style/named/foreign deletion-only fences, поскольку
   semantic batch не отправляется; оставшийся protected/ambiguous fence не
   обходится. Cleanup failure называет literal
   canary и ручное recovery. Guard старого anchor из одного code point (ASCII
   или emoji) срабатывает до shortcut и отказывает даже для true no-op.
   ...
   **Ответ во вкладке тоже запирай флагом.** `comments` отдаёт треды всего
   документа одним плоским списком, дополняя их осторожной read-only
   атрибуцией по цитате. Если документ многовкладочный, передавай доказанный
   `tab_id` и в `reply`:

   ```
   skrepka reply <doc_id> <comment_id> "Текст ответа" --tab <id>
   ```

   skrepka уже независимо сверит точный DOCX-якорь треда с выбранной вкладкой
   и проверит свежесть экспорта перед записью. Отказ не обходи и другой tabId
   не угадывай. `unknown` не превращай в `--tab` по первому кандидату.
   Комментарий уровня всего документа без текстового якоря обрабатывается без
   `--tab`; явно приписывать его вкладке нельзя.
...
из-за соседнего комментария, именованного диапазона, совпадения текста в
оглавлении или перевода строки в новом тексте. В отказе есть ссылка `?disco=` на мешающий тред — дай её
