...
remedy: обычно это либо мелкие точечные правки через `patch`, либо деструктивная
замена — тогда объясни последствия, дождись явного «да» и выполни её сам с
`--acknowledge-loss`. Команду человеку для самостоятельного запуска не передавай.
