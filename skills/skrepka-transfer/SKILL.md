...
1. **`patch`** — правки в документ с комментариями вносит он. С 0.10.0 он умеет
   ...
   Для предварительной проверки используй `patch --dry-run`: это advisory
   read-only receipt без записи. `unknown`/`not_simulated` нельзя выдавать за
   успешную будущую правку; exit 3 требует показать человеку причины.
